...
import { Address, Header, SessionIndex, ValidatorId } from '@polkadot/types/interfaces';
...
export class Subscriber {
  ...
  private async _validatorStatusHandler(): Promise<void> {
    ...
    this.validators.forEach(async account => {
      ...
      if (validatorActiveSetIndex < 0) {
        this.logger.debug(`Target ${account.name} is not present in the validation active set of session ${this.sessionIndex}`);
  ...
  private async _newSessionEventHandler(): Promise<void> {
    ...
    // Refresh validator set on every session change
    this.validatorActiveSet = await this.api.query.session.validators();
    this.logger.info(`New session ${this.sessionIndex}: updated validator active set`);
    await this._initValidatorsControllers();
  }
